...
Camera::Camera(const char *n, int w, int h, int f)
{
    ...
    sprintf(system_call_buffer,"su -c \"chmod 666 %s\"",  name);
...
{
	struct v4l2_control control;
	control.id = 1; //V4L2_CID_EXPOSURE_AUTO;
	control.value = 1 ; //V4L2_EXPOSURE_APERTURE_PRIORITY;
...
{
	struct v4l2_control control;
	control.id = 1 ; //V4L2_CID_CAMERA_CLASS_BASE+1;
	control.value = 1;
...
{
	struct v4l2_control control;
	control.id = 1 ; //V4L2_CID_EXPOSURE_AUTO_PRIORITY;
	control.value = v;
